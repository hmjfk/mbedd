...
6. 特定の場合で使用する命名規則は次の通りである。これは何重にも入り組んだ環境分岐を排除し、可読性を大幅に高めるためである。
    ...
    - 規格上関数名が規定されていても、C側実装によってはmacroなどで名前が変更されていることがある。これを吸収するために**pragma**(**mangle**)を用いる際の手順はつぎのようにする。
        1. **version**文で変更すべき識別子名を文字列定数にする。この際用いる識別子は`syn_`から始め、その後は対象の関数名とする。
        2. 関数宣言に**pragma**(**mangle**, syn_*funcname*)を付加する。
7. 識別子の使用にあたっては、処理系の最適化や`@live`属性の適用を考慮し、局所性を高めること。大域変数はABIに影響がない限り、禁止する。
8. できるだけ構造化プログラミングや関数型プログラミングにおける参照透過性と組込機能の概念（D言語でいう**pure**）を中心とすること。もし、他の考え方で書く場合は、C言語に落とし込めるような最低限の使用にどどめる。どうしても依存せざるを得ない場合は、実行記録体やRAM消費量の大きさに対する影響度合いに注意する。inline関数や**mixin template**の扱いも同様とする。
9. 部品化を促進するため、druntime及びphobosを取り込まないようにすること。そのため、runtimeを必要とする言語機能は一切使用できない。参考までに、使用できないmoduleを次に示す。
...
14. 説明文書で説明すべき事項は次の通りである。ただし、内部実装の説明を説明文書に載せる必要はない。
    ...
    -  mbeddは、次の事項を記載する。
        ...
        - その他、規格上文章化すべき事項
## 移植の指針
> 現在、C++ **class**型の構築と破棄に関しては研究中であり、この指針は実験的なものである。そのため、一部に誤りが含まれている可能性があることに注意を喚起する。
...
3. 派生型に対して用いる総称型は、ABIに影響が出ない限り、D形式の総称型でもよい。
4. 部品集内部で使用されるものを除き、C++の名前空間をDの名前空間単位と一致させること。
...
### 歴史的事項：mbedd.stdcpp名前空間とstd名前空間の対応
